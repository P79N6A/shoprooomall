...
// 		}
		...
		$site_dir_api = 'https://au.api.fastway.org/latest/tracktrace/massdetail/'. $order .'?api_key=64edc877bdc37768f9e0a834e7137a77';
		$result = $this->getcontent($site_dir_api);
		$result = json_decode($result);
...
$result = array_reverse($result['result']);
$r_error = '';
$scans = '';
...
if (empty($r_error)) {
    $data = array_reverse($scans);

	foreach ($data as $key => $val) {
		$express_info .= '<tr style="height:20px;">';
		$express_info .= '<td style=\'text-align:right;width:140px;\'>' . $val['Date'] . '</td>';
		$express_info .= '<td>&nbsp;&nbsp;|&nbsp;&nbsp;</td>';
		$express_info .= '<td style=\'text-align:left;\'>'. $val['RealDateTime'] . $val['Name']. $val['StatusDescription'] . '</td>';
		$express_info .= '</tr>';
	}
	...
	$express_info = '<span style="font-size:14px;">很抱歉，暂时无法查询此订单信息！</span>';
}
