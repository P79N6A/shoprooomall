...
jQuery.levelLink = function (){
	...
	$(document).on('click',liv,function(){
		...
		if(old_val != value){
                        if(type == 1){
                                $('#dlProvinces').children('.txt').html("省份");
				$('#dlProvinces').find('.options').html('<span class="liv hide" data-text="邮政编码" data-value="0">邮政编码</span>');
				$('#selProvinces').val(0);
                                $('#dlCity').children('div.txt').html("邮政编码");
				$('#dlCity').find('.options').html('<span class="liv hide" data-text="邮政编码" data-value="0">邮政编码</span>');
				$('#selCities').val(0);
				$('#dlRegion').find('.txt').html("市区");
				$('#dlRegion').find('.options').html('<span class="liv hide" data-text="市区" data-value="0">市区</span>');
				$('#selDistricts').val(0);
                        }else if(type == 2){
				$('#dlCity').children('div.txt').html("邮政编码");
				$('#dlCity').find('.options').html('<span class="liv hide" data-text="邮政编码" data-value="0">邮政编码</span>');
